...
dpkg-deb -b kali-src/ chip.deb # Build the deb package
dpkg -i chip.deb || (apt-get -f install && dpkg -i chip.deb) # this will automatically install the required packages
